...
global parent_path
current_path = os.path.dirname(os.path.abspath(__file__)) ##当前文件路径
parent_path = os.path.dirname(current_path)
parent_path = os.path.dirname(parent_path) ## 这个应该就是cosyvoice根目录
...
class CosyVoice:
    ...
    def inference_sft(self, tts_text, spk_id,new_dropdown):
        ...
        for i in self.frontend.text_normalize(tts_text, split=True):
            ...
            #     f.write(str(model_input))
            if new_dropdown != "无":
                new_drop_down_path = os.path.join(parent_path, f'voices/{new_dropdown}.pt')
                # 加载数据
                print(f"尝试从{new_drop_down_path}读取{new_dropdown}.pt")
                newspk = torch.load(new_drop_down_path)
                # with open(f'./voices/{new_dropdown}.py','r',encoding='utf-8') as f:
                #     newspk = f.read()
                #     newspk = eval(newspk)
                model_input["flow_embedding"] = newspk["flow_embedding"]
