...
from utils.chatgpt_new import Chatgpt
from utils.tts_new import MY_TTS
...
def start_asr_service():
    global asr_process
    # 启动 start_asr_service.py
    asr_process = subprocess.Popen(["python", "utils/start_asr_service.py"])
...
def send_msg(audio, text):
    try:
        ...
        if audio:
            ...
            stt_ret = chatgpt.ASR(output_file)
        ...
        audio_path = my_tts.text_to_speech(data)
...
if __name__ == "__main__":
    ...
    # # 在程序开始时启动 start_asr_service.py
    # start_asr_service()
    # 启动 Gradio
    demo.launch(share=config.get("gradio", "share"))
    # # 当 Gradio 应用结束时，确保关闭 start_asr_service.py
    # stop_asr_service()
